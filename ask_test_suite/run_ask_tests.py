...
"""
    ...
    # Placeholders in contains/not_contains, resolved from the real data so expectations don't
    # go stale when a season wraps up. {LATEST_FINISHED_SEASON} = newest season with Playoff
    # Standings rows (same definition as latestFinishedSeason() in the dashboard).
    with open(raw_path, encoding='utf-8') as f:
        raw_rows = json.load(f)
    placeholders = {
        '{LATEST_FINISHED_SEASON}': str(max(r['season'] for r in raw_rows if r['category'] == 'Playoff Standings')),
    }
